...
project {
    ...
    params {
        ...
        text("github_repository_name", "modlog4j", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `modlog4j` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "modlog4j", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
        text("env.PUBLISHED_JAVA_GROUP", "org.mcmodlauncher", label = "Published group", description = "The maven coordinate group that has been published by this build. Can not be empty.", allowEmpty = false)
    }

    features {
        githubIssues {
            id = "modlog4j__IssueTracker"
            displayName = "McModLauncher/modlog4j"
            repositoryURL = "https://github.com/McModLauncher/modlog4j"
        }
    }
}
